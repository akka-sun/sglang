...
class Scheduler(
    ...
    def init_cache_with_memory_pool(self):
        ...
        if (
            ...
            elif server_args.enable_selective_prefix_cache:
                from sglang.srt.mem_cache.selective_prefix_cache import (
                    SelectivePrefixCache,
                )

                sensitive_patterns = None
                if server_args.selective_cache_sensitive_patterns:
                    sensitive_patterns = [
                        p.strip()
                        for p in server_args.selective_cache_sensitive_patterns.split(",")
                        if p.strip()
                    ]

                self.tree_cache = SelectivePrefixCache(
                    params=params,
                    public_extra_key=server_args.selective_cache_public_extra_key,
                    sensitive_patterns=sensitive_patterns,
                    force_private_without_text=(
                        server_args.selective_cache_force_private_without_text
                    ),
                    enable_async_privacy_detector=(
                        server_args.enable_async_privacy_detector_process
                    ),
                    detector_queue_size=server_args.async_privacy_detector_queue_size,
                )
            else:
