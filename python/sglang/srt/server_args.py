...
class ServerArgs:
    ...
    enable_selective_prefix_cache: bool = False
    selective_cache_public_extra_key: str = "__public__"
    selective_cache_sensitive_patterns: Optional[str] = None
    selective_cache_force_private_without_text: bool = True
    enable_async_privacy_detector_process: bool = True
    async_privacy_detector_queue_size: int = 8192
    ...
    def add_cli_args(parser: argparse.ArgumentParser):
        ...
        parser.add_argument(
            "--enable-selective-prefix-cache",
            action="store_true",
            help="Enable selective public/private radix cache. Sensitive requests are isolated in private cache while non-sensitive requests can use public cache.",
        )
        parser.add_argument(
            "--selective-cache-public-extra-key",
            type=str,
            default=ServerArgs.selective_cache_public_extra_key,
            help="Namespace key used by the selective prefix cache public tree.",
        )
        parser.add_argument(
            "--selective-cache-sensitive-patterns",
            type=str,
            default=ServerArgs.selective_cache_sensitive_patterns,
            help="Comma-separated regex list for sensitive prompt detection in selective prefix cache.",
        )
        parser.add_argument(
            "--selective-cache-force-private-without-text",
            dest="selective_cache_force_private_without_text",
            action="store_true",
            help="Route requests without prompt text to private cache when selective prefix cache is enabled.",
        )
        parser.add_argument(
            "--no-selective-cache-force-private-without-text",
            dest="selective_cache_force_private_without_text",
            action="store_false",
            help="Allow requests without prompt text to use public cache in selective prefix cache mode.",
        )
        parser.set_defaults(
            selective_cache_force_private_without_text=ServerArgs.selective_cache_force_private_without_text
        )
        parser.add_argument(
            "--enable-async-privacy-detector-process",
            dest="enable_async_privacy_detector_process",
            action="store_true",
            help="Enable asynchronous privacy detector process scaffold for selective prefix cache.",
        )
        parser.add_argument(
            "--disable-async-privacy-detector-process",
            dest="enable_async_privacy_detector_process",
            action="store_false",
            help="Disable asynchronous privacy detector process scaffold.",
        )
        parser.set_defaults(
            enable_async_privacy_detector_process=ServerArgs.enable_async_privacy_detector_process
        )
        parser.add_argument(
            "--async-privacy-detector-queue-size",
            type=int,
            default=ServerArgs.async_privacy_detector_queue_size,
            help="Queue size for async privacy detector task/result queues.",
        )
        parser.add_argument(
            "--cuda-graph-max-bs",
            type=int,
