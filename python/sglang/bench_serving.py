...
def sample_openai_requests(
    ...
    - "messages": list of {"role": str, "content": str}
      OR
    - "user_turns": list[str], where each item is one user turn in a multi-turn chat
    ...
    for data in dataset:
        ...
        user_turns = data.get("user_turns", None)
        if isinstance(user_turns, list) and len(user_turns) > 0:
            cleaned_turns = [str(t).strip() for t in user_turns if str(t).strip()]
            if not cleaned_turns:
                continue
            prompt_len = len(
                tokenizer.apply_chat_template(
                    [{"role": "user", "content": cleaned_turns[0]}],
                    tokenize=True,
                    add_generation_prompt=True,
                )
            )
...
async def benchmark(
    ...
    benchmark_input_requests = input_requests
    if (
        args.dataset_name != "mooncake"
        and warmup_requests > 0
        and len(input_requests) > warmup_requests
    ):
        warmup_sources = input_requests[:warmup_requests]
        benchmark_input_requests = input_requests[warmup_requests:]
    else:
        warmup_sources = input_requests[: min(warmup_requests, len(input_requests))]

    print(f"Starting warmup with {len(warmup_sources)} sequences...")
    ...
    if args.dataset_name == "mooncake":
        if warmup_requests > 0 and len(input_requests) > 0:
        ...
        for warmup_request in warmup_sources:
            warmup_input = RequestFuncInput(
                model=model_id,
                prompt=warmup_request.prompt,
                api_url=api_url,
                prompt_len=warmup_request.prompt_len,
                output_len=min(warmup_request.output_len, 32),
                lora_name=lora_name,
                image_data=warmup_request.image_data,
                extra_request_body={**extra_request_body, **warmup_request.extra_request_body},
            )
            warmup_tasks.append(
                asyncio.create_task(request_func(request_func_input=warmup_input))
            )

    warmup_outputs = await asyncio.gather(*warmup_tasks) if warmup_tasks else []
    if is_multi_turn:
        warmup_outputs = [x for output in warmup_outputs for x in output]

    # Check if at least one warmup request succeeded
    if warmup_requests > 0 and warmup_outputs and not any(
        ...
        print(
            f"Warmup completed with {len(warmup_sources)} sequences. Starting main benchmark run..."
        )
    ...
    pbar_total = len(benchmark_input_requests)
    if (
        ...
        request_generator = get_mooncake_request_over_time(
            benchmark_input_requests,
            tokenizer,
            mooncake_slowdown_factor,
            mooncake_num_rounds,
        )
        print(
            f"Starting Mooncake trace replay. Sessions: {len(benchmark_input_requests)}, Rounds per session: {mooncake_num_rounds}. Slowdown factor: {mooncake_slowdown_factor}"
        )
        pbar_total *= args.mooncake_num_rounds
    else:
        request_generator = get_request(benchmark_input_requests, request_rate)
    ...
    metrics, output_lens = calculate_metrics(
        input_requests=None if is_multi_turn else benchmark_input_requests,
        outputs=outputs,
